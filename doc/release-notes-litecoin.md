SampleCoin Core version *0.15.1* is now available from:

  <https://download.cryptogera.com/samplecoin-0.15.1/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://git.cryptogera.com/cryptogera/samplecoin/issues>

To receive security and update notifications, please subscribe to:

  <https://groups.google.com/forum/#!forum/samplecoin-dev>
...
installer (on Windows) or just copy over `/Applications/SampleCoin-Qt` (on Mac)
or `samplecoind`/`samplecoin-qt` (on Linux).
...
SampleCoin Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

SampleCoin Core should also work on most other Unix-like systems but is not
...
A number of changes to the way SampleCoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 mSCO per kB (0.00001 SCO). This is to prepare for dropping the minimum transaction fee to 0.1 mSCO per kB (0.0001 SCO) in the next release.
